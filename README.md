...
- **해결**: `ResourceController` 스크립트의 `Update()` 안에서 `timeSinceLastChange` 변수가 시간에 따라 변하게 함으로써 해결했습니다. 변수의 값이 변하게 되자, 동시에 애니메이터에 포함된 `IsDamage` ㄱ밧 역시 변하게 되면서, 몬스터가 데미지를 입었을 때 데미지 입는 애니메이션이 무한히 재생되는 문제도 함께 해결되었습니다.
